<!--<!DOCTYPE html>-->
<!--<html lang="en">-->

 
        <title>Login</title>
         <center style="background-color:#000000;">

       <embed width="800" height="300" src="images.jpg">
		 </center>

        
        

<body>


<table width="1350px">



								



			<tr>
				<td style="background-color:#D6CC94; width:300px; height:1000px; float:left;">


							 <!--<div class="modal-content">
							 <div class="modal-header">-->

							 <h3 class=" text-center"><b>Login</b></h3> 


							<!-- <div class="modal-body">-->

							<form action="checking.php" method="post">



								<table class="table"  align="center" > 


										<tr>
											<td>Name</td>
											<td><input type="text" placeholder="Name" name="name_id" id="name_id" required></td>
										</tr>


										<tr>
											<td>Password</td>
											<td><input type="password" name="password" id="password" placeholder="password" required></td>
										</tr>
								</table>

							<center>
										<input type="submit" value="login" name="login" id="login" class="btn"></a><br>

							</center>


										<font  size="3"><span class="pull-right" ><a href="index.php?option=register">Register</a></span></font>
										<br><br>

										<embed width="300" height="200" src="https://www.suninnshotel.com.my/images/kepong_map.jpg">
										<br>
										
										<embed width="300" height="200" src="http://media.expedia.com/hotels/8000000/7420000/7414500/7414420/7414420_69_b.jpg"><br>
										<embed width="300" height="200" src="http://1.bp.blogspot.com/-uwDxpXa1JSY/Tw-awaWe4oI/AAAAAAAAAHg/zNHupFjXLgM/s1600/IMG_0661.JPG">


							</form>
							
							<!--
							</div>

							</div> -->




							            	
				
				</td>

									<td style="background-color:#eeeeee;height:900px;width:1050px;">
									<center>
											<embed width="800" height="400" src="http://www.klhotels.net/pics/sunwayputrakl_sitemap.jpg">
									</center>		
											 <center>Welcome to our Hotel Putra website. Visit us and we are ready to serve you.</center>
											
									</td>

			</tr>



			<tr>
				<td colspan="2" style="background-color:#796541;text-align:center;"> Copyright by amtis.com </td>
			
			</tr>
	</table>

</body>
<!--
</html>-->
